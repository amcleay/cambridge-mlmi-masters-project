...
def dict2str(x, colored=False):
    ...
    for key, value in x.items():
        ...
        if colored:
            output.append(
                f"{bcolors.YELLOW}{key}{bcolors.ENDC}={bcolors.GREEN}{value}{bcolors.ENDC}"
            )
        else:
...
def dict2list(x):
    ...
    for key, value in x.items():
        ...
        if isinstance(value, str):
            ...
            value, list
        ):  # only 1.8 turns, usually on system side, have multiple values for a slot
...
def compare_slot_values_in_state(slot_values1, slot_values2):
    ...
    for slot in (
        slot_values1.keys() & slot_values2.keys()
    ):  # check slots in intersection to see if their value changed
        values1 = slot_values1[slot]
        values2 = slot_values2[slot]
        if (
            len(set(values1) & set(values2)) == 0
        ):  # none of values matched between two value lists
            return True
    return False
...
def show_turn_meta(turn_meta):
    print(
        f"intent: {bcolors.RED}{turn_meta.usr_intent}{bcolors.ENDC}, start_new_task: \
        {bcolors.OKBLUE}{turn_meta.start_new_task}{bcolors.ENDC}, goal_change: \
        {bcolors.OKCYAN}{turn_meta.goal_change}{bcolors.ENDC}"
    )
...
def get_special_tokens():
    ...
    print(
        "Load special tokens: {}".format(
            len(SPECIAL_TOKENS["additional_special_tokens"]) + 4
        )
    )
