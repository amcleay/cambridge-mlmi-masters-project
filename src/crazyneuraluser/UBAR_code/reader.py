...
class _ReaderBase(object):
    ...
    def get_batches(self, set_name):
        ...
        for k in turn_bucket:
            ...
            log_str += (
                "turn num:%d, dial num: %d, batch num: %d last batch len: %d\n"
                % (
                    k,
                    len(turn_bucket[k]),
                    len(batches),
...
class MultiWozReader(_ReaderBase):
    def __init__(self, tokenizer):
        ...
        test_list = [
            test_list.strip().lower()
            for test_list in open(cfg.test_list, "r").readlines()
        ]
        dev_list = [
            dev_list.strip().lower() for dev_list in open(cfg.dev_list, "r").readlines()
        ]
    ...
    def _build_vocab(self):
        ...
        vp = (
            cfg.vocab_path_train
            if cfg.mode == "train" or cfg.vocab_path_eval is None
            else cfg.vocab_path_eval
        )
    ...
    def _construct_aspn_constraint(self):
        ...
        aspn_masks = {}
        all_dom_codes = [
            self.vocab.encode("[" + d + "]") for d in ontology.dialog_acts.keys()
        ]
        all_act_codes = [
            self.vocab.encode("[" + a + "]") for a in ontology.dialog_act_params
        ]
        ...
        for a in all_act_codes:
            aspn_masks[a] = (
                all_dom_codes + all_slot_codes + [self.vocab.encode("<eos_a>")]
            )
        ...
        if save_temp:  # save encoded data
            if "all" in cfg.exp_domains:
                encoded_file = os.path.join(
                    cfg.data_path, "new_db_se_blank_encoded.data.json"
                )
            ...
            if os.path.exists(encoded_file):
                ...
                self.data = json.loads(
                    open(cfg.data_path + cfg.data_file, "r", encoding="utf-8")
                    .read()
                    .lower()
                )
            ...
            if cfg.multi_acts_training:
                ...
                if fn in self.multi_acts:
                    ...
                    for act_type, act_spans in turn_ma.items():
                        enc["aspn_aug"].append(
                            [
                                self.tokenizer.encode(a.split() + ["<eos_a>"])
                                for a in act_spans
                            ]
                        )
        ...
        with open(cfg.eval_load_path + "/dialogue_record.csv", "w") as rf:
            ...
            for dial_id in ordered_dial:
                ...
                for turn_no, turn in enumerate(turns):
                    ...
                    domain = [
                        i[1:-1] for i in self.dspan_to_domain(turn["dspn"]).keys()
                    ]
