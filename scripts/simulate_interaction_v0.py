...
def main(
    ...
    user_utterances_out_path = (
        "data/preprocessed/UBAR/TEST_user_utterances_from_simulator.txt"
    )
    ...
    for dialogue_idx, (goal, dialogue_filename) in enumerate(
        ...
        curr_dialogue_user_utterances_formatted = []
        if test_only:
            if dialogue_filename not in test_list:
                continue
...
if __name__ == "__main__":
    ...
    if ground_truth_system_responses == "False":
        ...
        ground_truth_system_responses = True
    main(
        write_to_file=True, ground_truth_system_responses=ground_truth_system_responses
    )
