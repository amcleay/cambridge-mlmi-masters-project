...
def generation_func(model, input_ids, eos_id, dec_max_len):
    ...
    for i in range(dec_max_len):
        ...
        for bs_idx, token_id in enumerate(next_token):
            if (
                finish_sent[bs_idx] is False and token_id.item() == eos_id
            ):  # first produce <eos>
...
class NeuralAgent:  # crazyusermodel
    ...
    def prepare_input_ids(self, data: dict, start_token: str) -> str:
        ...
        if transformers.__version__.startswith(
            "2."
        ):  # compatible with transformers v2.x used in convlab2
    ...
    def update_internal_data(self, data: dict) -> None:
        ...
        # update context
        sys_utt_wrap = wrap_element(
            "SYS", data["SYS_UTT"]
        )  # e.g., <SYS/> Which area would you prefer? </SYS>
        usr_utt_wrap = wrap_element(
            "USR", data["USR_UTT"]
        )  # e.g., <USR/> I want to be in the centre. </USR>
        self._context_str = add_str(self._context_str, sys_utt_wrap)
        self._context_str = add_str(self._context_str, usr_utt_wrap)

        # update prev usr act
        self._prev_usr_act = data[
            "USR_ACT"
        ]  # e.g., <ACT/> inform </ACT> <SLOT/> area </SLOT> <VALUE/> centre </VALUE>

    def run_inference_once(self, input_ids: torch.tensor, eos_id: int) -> List:
        if transformers.__version__.startswith(
            "2."
        ):  # compatible with transformers v2.x used in convlab2
            output = generation_func(
                self.model, input_ids, eos_id, self.config.decode.dec_max_len
            )
        else:
    ...
    def generate_whole_sequence(self, sys_utt: str) -> tuple:
        ...
        generation = self.tokenizer.decode(
            output[0]
        )  # decode back to str, including the fed context
    ...
    def _prepare_current_constraints(
        ...
        for intent in involved_intents:
            ...
            for slot, value_list in self.complete_goal["constraints"][intent][
                "informable"
            ].items():
            ...
            # requestable
            if if_reset_reqt:
                constraints["requestable"] = copy.deepcopy(
                    self.complete_goal["constraints"][intent]["requestable"]
                )
            else:
                constraints["requestable"] = copy.deepcopy(
                    self.current_constraints[intent]["requestable"]
                )
    ...
    def prepare_turn_goal(
        self, prev_usr_act: str, sys_act: str, SNT_flag: str, GC_flag: str, RA_flag: str
    ) -> str:
    ...
    def _update_current_constraints(
        self, intent: str, spk: str, usr_act: str, sys_act: str
    ) -> None:
        ...
        if spk == "sys":
            ...
            if self.const_act_str["fail_search"] in act_dict:
                ...
                keep_slot_types = [
                    self.slot_types["book"]
                ]  # already found entities, no need to keep search slots
                self._prepare_current_constraints(
                    [intent], keep_slot_types, if_reset_reqt=False
                )
            ...
            # when the system request #
                ...
                for slot in requested_slots.keys():
                    ...
                    if (
                        slot
                        in self.complete_goal["constraints"][intent][
                            "informable"
                        ].keys()
                    ):  # dict of slot to value_list
                        slot_type = self._get_slot_type(slot)
                        value_idx = self.user_status["constraint_idx"][intent][
                            slot_type
                        ]
                        value = self.complete_goal["constraints"][intent]["informable"][
                            slot
                        ][value_idx]
            ...
            if self.const_act_str["inform"] in act_dict:
                for slot, value_list in act_dict[self.const_act_str["inform"]].items():
                    ...
                    for (
                        ...
                        value_list
                    ):  # possible to have multi-value slots in user act in corpus
                        if (
                            self.finish_inform == "loose"
    ...
    def parse_act(act_seq: str) -> dict:
        ...
        for act_seg in act_seq:
            ...
            act_seg = " ".join(
                act_seg[end_idx + 1 :]
            )  # act arguments (slot/value pairs)
            # print(f"act: {act}\n", act_seg, "\n")
